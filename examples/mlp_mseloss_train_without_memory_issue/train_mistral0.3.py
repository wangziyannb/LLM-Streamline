...
if __name__ == '__main__':
    ...
    wandb.init(
        project="slimpajama_prune_mistral0.3",
        config={
            "epochs": 20,
            "batch_size": 32,
            "learning_rate": 1e-3,
    ...
    model = MistralModel(auto_config)
    mistral_model = AutoModelForCausalLM.from_pretrained(config.model_name)

    # Load pretrained weights into the replace_layer model
    state_dict = model.state_dict()
    mistral_dict = mistral_model.state_dict()
    state_dict['embed_tokens.weight'] = mistral_dict['model.embed_tokens.weight']
    for i in range(auto_config.num_hidden_layers):
        for proj in ['q_proj', 'k_proj', 'v_proj', 'o_proj']:
            state_dict[f'layers.{i}.self_attn.{proj}.weight'] = mistral_dict[f'model.layers.{i}.self_attn.{proj}.weight']
        for mlp_proj in ['gate_proj', 'up_proj', 'down_proj']:
            state_dict[f'layers.{i}.mlp.{mlp_proj}.weight'] = mistral_dict[f'model.layers.{i}.mlp.{mlp_proj}.weight']
        for ln in ['input_layernorm', 'post_attention_layernorm']:
            state_dict[f'layers.{i}.{ln}.weight'] = mistral_dict[f'model.layers.{i}.{ln}.weight']
    ...
    if os.path.exists("slimpajama-Mistral-3-tokenized-0.06b"):
        datasets = load_from_disk("slimpajama-Mistral-3-tokenized-0.06b")
        ...
        ds.save_to_disk("slimpajama-Mistral-3-tokenized-0.06b")
